...
//	debugImage_ = cv_ptr->image * 0.2;
	...
	fstream stream((fname + ".txt").c_str(),ios::out);
	stream <<"# u v x y z"<<endl;
	for(int y=0;y<cv_ptr->image.rows;y++) {
		for(int x=0;x<cv_ptr->image.cols;x++) {
			cv::Point3f p = to3D(cv::Point(x,y));
			if(isfinite(p.x))
				stream <<x<<"\t"<<y<<"\t"<<p.x<<"\t"<<p.y<<"\t"<<p.z<<endl;
		}
	}
	stream.close();
	cv::imwrite(fname + "-depth.png", debugImage_*256);

	fstream stream2((fname + "-depth.txt").c_str(),ios::out);
	stream <<"# per pixel depth in meter, cols x rows ="<<cv_ptr->image.cols<<"x"<<cv_ptr->image.rows<<endl;
	for(int y=0;y<cv_ptr->image.rows;y++) {
		for(int x=0;x<cv_ptr->image.cols;x++) {
			cv::Point p = cv::Point(x,y);
			stream2 << cv_ptr->image.at<float> (p)<<" ";
		}
		stream2 << endl;
	}
	stream2.close();
...
void imageRGBCallback(const sensor_msgs::Image::ConstPtr& msg) {
	cout << "received RGB image" << endl;
	cv_ptr = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::BGR8);
	cv::cvtColor(cv_ptr->image,cv_ptr->image,CV_RGB2BGR);
	cv::imwrite(fname+"-color.png",cv_ptr->image);

	fstream stream((fname + "-color.txt").c_str(),ios::out);
	stream <<"# pixel-wise (r g b), cols x rows ="<<cv_ptr->image.cols<<"x"<<cv_ptr->image.rows<<endl;
	for(int y=0;y<cv_ptr->image.rows;y++) {
		for(int x=0;x<cv_ptr->image.cols;x++) {
			cv::Point p = cv::Point(x,y);
			stream << (int)cv_ptr->image.at<cv::Vec3b>(p)[2] << " ";
			stream << (int)cv_ptr->image.at<cv::Vec3b>(p)[1] << " ";
			stream << (int)cv_ptr->image.at<cv::Vec3b>(p)[0] << " ";
		}
		stream << endl;
	}
	stream.close();
...
int main(int argc, char** argv) {
	ros::init(argc, argv, "filewriter");

	if(argc != 2) {
		cout <<"wrong number of arguments. Usage: filewriter <prefix>"<<endl;
		exit(0);
	}
	fname = string("kinect-")+argv[1];

	ros::NodeHandle n;

	ros::Subscriber sub = n.subscribe("camera/depth/image", 1, imageCallback);
	ros::Subscriber sub3 = n.subscribe("camera/rgb/image_color", 1, imageRGBCallback);
